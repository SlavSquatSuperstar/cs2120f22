...
| 0 := 0
| 1 := 1
| (n' + 2) := fib n' + fib (n' + 1) -- sum the previous two terms
...
rw involutive,
assume a : bool,
cases a with t f, -- easy since we have two values
exact rfl, -- just evaluate each
exact rfl,
end 
...
begin
-- show there must be an x, where x*x = 25
...
example : 4 ∈ (X ∩ Y) :=
begin
show 4 ∈ X ∧ 4 ∈ Y, -- prove 4 ∈ X and 4 ∈ Y
apply and.intro,
-- prove 4 ∈ X
right,
right,
...
-- 4 is not in the set difference, X \ Y
-- Set difference: X \ Y (everything in X and not in Y)
-- 4 is not in the intersection of X and the complement of Y
example : 4 ∉ X ∩ Yᶜ :=
begin
assume in_diff,
apply and.elim in_diff,
assume in_x, -- 4 ∈ X
assume in_yc, -- 4 ∈ Yᶜ
let in_y : 4 ∈ Y := _, -- 4 ∈ Y
contradiction,
...
-- every a maps to only one b
-- if r(a) = b and r(a) = c, then b = c
